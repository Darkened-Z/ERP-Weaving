...
- **Live than serials** (`src/components/than-serial-live.tsx` `ThanSerialLive`): **VOUCHER-MATCHED per owner** — every active COUNT row of one voucher shares the SAME serial, the voucher's own monthly number with **3-digit padding** (`SEP-001-26`, was `SEP-0001-26` per-row increments). Server: one monthly seq per voucher (MAX+1 in tx), stamped on every active row lacking a serial. Per-row delivery memory on edit is keyed `serial::beamNo` so shared serials don't cross-mark A/B/C thans as delivered. Grey-despatch than consumption lists one pending row per (serial, row) — the operator picks the right grade row.
