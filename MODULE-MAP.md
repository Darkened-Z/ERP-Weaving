...
- **"Sale Voucher" picker (field `purContNo`, F9) + Conv contract picker** — the Pur.Cont No slot now attaches an **EXTERNAL YARN SALE VOUCHER (YSV-)** as optional info (owner change: it used to list yarn purchase contracts). Optional — receipts save without one; picking fills only count + brand (info). The int+ext purchase-contract union query survives solely as the party-count **rate fallback** in `pcMap`.
...
- **Beam-status guard (DP side)**: DP **delete and edit-drop** release their beams back to the knotting mount — a beam carrying `knVno` goes back to **KNOTTING** (mount survives), only never-knotted beams revert to EMPTY. (Deleting a production voucher used to force EMPTY and silently un-mount knotted beams — the same regression class fixed on the IWB side.) The auto "last roll → EMPTY" when Σ woven ≥ beam length is intentional and stays.
...
- Consumes the knotting mount: a loom's mounted beams + their contract flow into production.
- **REDESIGNED per owner (2026-09):** **BEAM DETAILS grid first** (Beam Set# · Type · K/S/M Date · Beam Status · Wast WT KG · Beam# (F9) · Cont No · Ends · B.Length · Rcvd/Mtr · Diff · Shrinkage, tbody `idp-beam-rows`), **COUNTS GRID below it** (mm/Than Sr No · A · B · C · CP · PPC · Total · Rej, tbody `idp-count-rows`); rows pair by index across the two tables. **PARTIES section lists ONLY own conversion parties** (`1.01.01.01.*` via `WVG_CONVERSION_PREFIX` — other contractors' accounts excluded, owner). **Set# column and header field REMOVED** (stored `set_no`/`set_hash` stay null on new rows; DB columns intact). **Loom# moved from the grid to the HEADER** (`headerLoom` FindingPicker, shed-scoped): picking it auto-fills **ALL of that loom's mounted beams** into the beam rows (`LoomBeamsFill` component — 1, 2, however many). **ALT+Z PROD DETAIL section REMOVED** (its table is no longer read or written). **Folding Stock is server-computed** (prod − despatch for the conv party), readonly, and auto-fills client-side from the picked conv party via `foldingByParty`. **Rcvd/Mtr / Diff / Shrinkage stay readonly auto** (per-beam cumulative incl. this voucher, computed live by the rewritten `ProductionSetCalc` across the two tables).
...
- **SET grid pickers** (`int_daily_production_set` gained `loom_no` + `cont_no`; both migrated to Turso):
  ...
  - **Beam # (F9)** = "SET NO LIST" `FindingPicker` name `beamNo` over RUNNING beams (Beam Set/Set No/Beam No/Set Status/Wrk/Ends/Length/Cont No). Existing `RowAutoFill watch=beamNo` (`beamFillMap`, now also fills `beamStatus`+`contNo`). **Scoped to the header Loom# (2026-09, owner)** via `filterByField="headerLoom"` + a row `filterKey` of `` `${shed}|${loomNo}` `` — the list offers only the beams mounted on THAT loom, not every mounted beam. A beam missing its shed/loom stamp gets an empty key and stays visible (never silently unpickable); an empty Loom# shows all, so edit mode is unaffected.
  - **Cont No** readonly cell (persisted `cont_no`).
- **PARTIES auto-fill + yarn spec (2026-09, owner):** picking a beam (`BeamPartyFill`) or a loom (`LoomBeamsFill`) fills **every** party box, not just Beam Cost Party. Beams carry no party columns in practice (`beams.party_trade` / `szg_party` are null in prod), so both fall back to the **warped-beam receiving bill** the beam came in on (`beams.brVno` → `int_warped_beam_receiving`): `bmSaleParty` → Beam Cost Party, `beamReceivingFrom` → Szg Party. Those are stored as COA **codes** there and are mapped to descriptions to match the Combobox values. **Szg Party now lists SIZING creditors** (head resolved by `%SIZING%COMMERCIAL%`, i.e. `3.03.06.02.*` — same head the IWB "Beam Receiving From" picker uses), not conversion debtors. The beam's contract is also pushed through `conv_contract` so Yarn Cost Party / quality / brand fill from one pick.
- **Yarn spec boxes in PARTIES:** readonly `beamWarpInfo` under Beam Cost Party (**warp only**), and `yarnReadPick` / `yarnWarpInfo` / `yarnWeftInfo` under Yarn Cost Party. Sourced from the contract head (`read` × `pick`) + its warp/weft count grids (`int|ext_grey_conv{ersion}_warp|weft` → `count. descr brand`, warp also `— ends E`). Filled live by the `conv_contract` `AutoFill`; on edit rendered server-side from the first set row's `cont_no`.
- **Rcvd/Mtr opening balance (2026-09, owner):** `beamStats[beamNo].rcvd` is the **Rcvd/Mtr the last saved voucher stamped on that beam** (rows read oldest-first, last write wins; current voucher excluded on edit), falling back to Σ(totalCount+rejCount) for beams whose older vouchers predate stored `rcvd_mtr`. `ProductionSetCalc` then adds this row's Total + Rej.
...
- **Live than serials** (`src/components/than-serial-live.tsx` `ThanSerialLive`): each active count row gets its own monthly number (3-digit) **plus its grade tag** — `SEP-001-26|A`, `SEP-002-26|B` … (grade = whichever of A/B/C/CP/PPC has meters in that row; client-approved format). Server mirrors it (per-row seq MAX+1 in tx + grade suffix). Per-row delivery memory on edit is keyed `serial::beamNo`. Grey-despatch than consumption matches the full serial incl. tag.
- **Grade-tag correction (2026-09):** `ThanSerialLive` re-tags any serial already in this voucher's own shape (`vNo` or `vNo/…`) whenever the grade column changes, so a row saved as `/A` that now carries B or C meters is corrected on load/edit. A serial the operator typed by hand is marked `data-manual` and never rewritten.
...
- **PRODUCTION THAANS panel** (`DesignThansFill` in `production-calc.tsx`, data from `inventory/grey-despatch/thans/route.ts`): triggered by a `combobox:change` on **`conv_cont_no`** — the route resolves the contract's party (int then ext table) and lists that party's undelivered thans. Picking/unpicking rows drives the line grid below via `fillLineGrid`.
- **Rows are keyed by `int_daily_production_set.id`, never by the serial (2026-09).** One production voucher's thans **share one `mm_than_sr_no`** (the grade tag is per row, not per than), so a serial-keyed `removed` set deleted both rows of a voucher at once and React saw duplicate keys. The route returns `id` for exactly this.
- **Beam#/Set# fallback:** daily production pairs a counts row with the beam row at the SAME index, so the 2nd, 3rd … than of a single-beam voucher is stored with `beam_no` NULL. The route fills those in from the voucher's beam **only when the voucher ran on exactly one beam** — multi-beam vouchers keep the blank rather than guess.
- **Totals:** panel header + tfoot show selected than count and Σ meters; the line grid has a `<tfoot>` TOTAL row (`#gd-tot-cnt|a|b|c|cp|rej|len`) filled by `DespatchAmountCalc` (Oracle's closing grid row). **Than / Qty and Qty Mtrs are readonly and auto-filled** from the grid — they are the same two numbers the `than_count` / `mtrs_mismatch` save guards check the grid against, so typing them was pure error surface. Qty Mtrs has **no DB column** (Σ line length; server-side default computed from the saved lines).
- **Line grid collapses** to the picked thans + one blank spare via `HideEmptyRows` on tbody `gd-line-rows` (the fixed 20-row block is gone). `DespatchAmountCalc` walks the DOM (`line_t_sr_${n}` until missing) rather than its `lineRows` prop, since a saved despatch can render more rows than the constant.
