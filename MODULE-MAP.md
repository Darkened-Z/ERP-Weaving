...
- **Amount = Beam Length × Ends (tar) ÷ 1693.20 ÷ Result Count SZG × Rate** — Rate = the ROW's own Rate when typed, else the header Sizing Rate (both client calc + server save). Result Count SZG required for the division. Formula note shown under Sizing Rate.
