...
export function ShellLayout({
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside
        className={`w-56 bg-[var(--sidebar-bg)] text-[var(--sidebar-fg)] border-r border-[var(--sidebar-border)] flex flex-col fixed h-screen overflow-y-auto scrollbar-thin z-50 transition-transform duration-200 ${
      ...
      >
        <div className="p-5 border-b border-[var(--sidebar-border)] flex items-center justify-between">
          ...
          <button
            onClick={() => setOpen(false)}
            className="lg:hidden text-[var(--sidebar-muted)] hover:text-[var(--sidebar-fg)] text-xl leading-none cursor-pointer p-1"
          >
            &times;
          </button>
        </div>
        ...
        <div className="p-5 border-t border-[var(--sidebar-border)]">
          <div className="text-[12px] font-medium">{sessionName}</div>
          <div className="text-[9px] uppercase tracking-[0.1em] text-[var(--sidebar-muted)] mt-0.5">
            {sessionRole}
          </div>
          <form action={logoutAction}>
            <button
              type="submit"
              className="mt-2 text-[10px] uppercase tracking-[0.08em] text-[var(--sidebar-muted)] hover:text-[var(--sidebar-fg)] transition-colors cursor-pointer"
            >
              Sign out
            </button>
          </form>
