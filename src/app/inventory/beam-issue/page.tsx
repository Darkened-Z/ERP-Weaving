...
export default async function EmptyBeamIssuePage({
  searchParams,
}: {
  ...
  const list = findFilter
    ? await db
        .select()
        .from(schema.intEmptyBeamIssue)
        .where(sql`
          ${schema.intEmptyBeamIssue.vNo} LIKE ${pat} ESCAPE '\\' OR
          ${schema.intEmptyBeamIssue.szgWrpParty} LIKE ${pat} ESCAPE '\\' OR
          ${schema.intEmptyBeamIssue.convParty} LIKE ${pat} ESCAPE '\\'
        `)
        .orderBy(desc(schema.intEmptyBeamIssue.id))
    : await db.select().from(schema.intEmptyBeamIssue).orderBy(desc(schema.intEmptyBeamIssue.id));
  ...
  const lines = editing
    ? await db
        .select()
        .from(schema.intEmptyBeamIssueLine)
        .where(eq(schema.intEmptyBeamIssueLine.issueId, editing.id))
        .orderBy(schema.intEmptyBeamIssueLine.srNo)
    : [];

  const upcomingVNo = nextVNoFromRows(list, "IBI");
  ...
  async function saveAction(formData: FormData) {
    ...
    const header = {
      ...
      statusType: txt(formData.get("statusType")),
    ...
    try {
      if (Number.isFinite(id) && id > 0) {
        await db.transaction(async (tx) => {
          await tx
            .update(schema.intEmptyBeamIssue)
            .set({ ...header, modifiedDate: nowIso })
            .where(eq(schema.intEmptyBeamIssue.id, id));
          await tx
            .delete(schema.intEmptyBeamIssueLine)
            .where(eq(schema.intEmptyBeamIssueLine.issueId, id));
          if (validLines.length) {
            await tx
              .insert(schema.intEmptyBeamIssueLine)
              .values(validLines.map((l) => ({ ...l, issueId: id })));
          }
        });
        revalidatePath("/inventory/beam-issue");
        redirect(`/inventory/beam-issue?id=${id}`);
      } else {
        const providedVNo = ((formData.get("vNo") as string) || "").trim();
        const newId = await db.transaction(async (tx) => {
          const existing = await tx.select({ vNo: schema.intEmptyBeamIssue.vNo }).from(schema.intEmptyBeamIssue);
          const vNo = providedVNo || nextVNoFromRows(existing, "IBI");
          const inserted = await tx
            .insert(schema.intEmptyBeamIssue)
            .values({ ...header, vNo, postedDate: nowIso })
            .returning({ id: schema.intEmptyBeamIssue.id });
          const insertedId = inserted[0].id;
          if (validLines.length) {
            await tx
              .insert(schema.intEmptyBeamIssueLine)
              .values(validLines.map((l) => ({ ...l, issueId: insertedId })));
          }
          return insertedId;
        });
        revalidatePath("/inventory/beam-issue");
        redirect(`/inventory/beam-issue?id=${newId}`);
      }
    } catch (e: unknown) {
      const msg = (e as { message?: string })?.message ?? "unknown";
      if (/UNIQUE|constraint/i.test(msg)) {
        redirect(`/inventory/beam-issue?error=code_exists`);
      }
      throw e;
    }
  }
  ...
  async function deleteAction(formData: FormData) {
    ...
    if (id === null) return;
    await db.transaction(async (tx) => {
      await tx.delete(schema.intEmptyBeamIssueLine).where(eq(schema.intEmptyBeamIssueLine.issueId, id));
      await tx.delete(schema.intEmptyBeamIssue).where(eq(schema.intEmptyBeamIssue.id, id));
    });
    revalidatePath("/inventory/beam-issue");
    redirect(`/inventory/beam-issue`);
  }
    ...
    <Shell active="beam-issue">
      <div className="animate-in">
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between mb-6 gap-4">
          <div>
            <h1 className="page-title">EMPTY BEAM ISSUED ( WVG )</h1>
          ...
          <ExcelExportButton
            ...
            filename="empty-beam-issue"
            sheetName="EmptyBeamIssue"
          />
        </div>
        ...
        <form id="ibi-find-form" method="GET" action="/inventory/beam-issue" className="hidden" />

        <div className="border border-black p-6 mb-6">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <div className="text-[11px] uppercase tracking-[0.1em] font-semibold">
              {isAdding ? "New — EMPTY BEAM ISSUED" : editing ? `Edit — ${editing.vNo}` : "EMPTY BEAM ISSUED"}
            </div>
            <div className="flex gap-2 no-print flex-wrap">
              <a href="/inventory/beam-issue?adding=1" className="btn btn-outline btn-sm">New</a>
              <button type="submit" form="ibi-save-form" className="btn btn-sm">Save</button>
              ...
              <a href="/inventory/beam-issue" className="btn btn-outline btn-sm">Exit</a>
            </div>
          </div>

          {showForm && (
            <form id="ibi-save-form" action={saveAction}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-x-4 gap-y-3">
                ...
                <div className="lg:col-span-2">
                  ...
                  <select name="statusType" className="input-box mono" defaultValue={editing?.statusType ?? ""}>
                    <option value=""></option>
              ...
              <div className="flex items-end gap-2 mt-6 no-print flex-wrap">
                ...
                <div className="ml-auto flex items-end gap-4">
                  <div className="mono text-[10px] text-[var(--muted)]">Note:- Only New Mode Beam Issue</div>
                  <div>
        ...
        <div className="border border-black">
          <div className="flex items-center justify-between px-4 py-3 border-b-2 border-black">
            ...
            <form className="flex gap-2" id="find-form" method="GET" action="/inventory/beam-issue">
            ...
            <table>
              ...
              <tbody>
                {list.map((r) => {
                  ...
                  const href = `/inventory/beam-issue?id=${r.id}`;
