...
export default async function IntGreyConversionContractPage({
  ...
  const warpRows = selected
    ? await db
        ...
        .where(eq(schema.intGreyConversionWarp.contractId, selected.id))
  ...
  const weftRows = selected
    ? await db
        ...
        .where(eq(schema.intGreyConversionWeft.contractId, selected.id))
        .orderBy(schema.intGreyConversionWeft.srNo)
    : [];

  // 4 rows like the external form, extended when an older internal contract
  // already carries rows beyond srNo 4 (the internal grid used to be 9-row) —
  // otherwise editing would silently drop those saved rows.
  ...
  // Internal conversion is inventory-only — the Party picker is limited to the
  // DEBTORS conversion heads (WVG 1.01.01.01 + COMMERCIAL 1.01.01.19) via the
  // shared helper; the rest of the form is identical to the external contract.
  ...
  async function saveContract(formData: FormData) {
    ...
    try {
      ...
      if (lockMatch) {
        redirect(`/inventory/contracts/grey-conversion?error=period_locked&thru=${lockMatch[1]}`);
      }
  ...
  async function deleteContract(formData: FormData) {
    ...
    if (s?.roleName !== "ADMIN") redirect("/inventory/contracts/grey-conversion?error=admin_only");
      ...
      <div className="animate-in">
        ...
        <div className="border border-black p-5 mb-6">
          ...
          <form id="igc-save-form" action={saveContract}>
            ...
            <div className="flex items-end gap-2 mt-5 flex-wrap">
              ...
              {formItem && (
                <a
                  href={`/inventory/contracts/grey-conversion/${formItem.id}/print`}
                  target="_blank"
        ...
        <div className="border border-black">
          <form action="/inventory/contracts/grey-conversion" method="get" className="flex gap-2 items-end flex-wrap border-b border-black p-3 bg-gray-50">
            ...
            {(findFilter || fParty || fGrey || fStatus) && <a href="/inventory/contracts/grey-conversion" className="btn btn-outline btn-sm">Clear</a>}
          </form>
          {selected && (() => {
