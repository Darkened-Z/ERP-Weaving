...
export default async function YarnPurchaseContractPrint({
  params,
}: {
  ...
  const sellerCode = contract.partyCode ?? "";
  let sellerName = sellerCode;
  if (sellerCode) {
    const [sellerAcc] = await db
      .select({ description: schema.chartOfAccounts.description })
      .from(schema.chartOfAccounts)
      .where(eq(schema.chartOfAccounts.code, sellerCode))
      .limit(1);
    if (sellerAcc?.description) sellerName = sellerAcc.description;
  }
  ...
  return (
    <>
      ...
      <div className="no-print p-3 flex gap-2 border-b border-black">
        <a href={`/external/contracts/yarn-purchase?id=${id}`} className="btn btn-outline btn-sm">
          Back
        </a>
        <PrintButton label="Print" />
      </div>
      <div className="p-6 mx-auto" style={{ maxWidth: 900 }}>
        <PrintHeader title="YARN PURCHASE CONTRACT" right={rightBlock} />
        ...
        <div className="grid grid-cols-2 gap-6 mb-4 text-[12px]">
          ...
          <div className="border border-black p-3">
            ...
            <div className="text-[11px]">Code: {sellerCode || "-"}</div>
          </div>
        </div>

        <p className="text-[12px] mb-4 leading-relaxed">
          This YARN PURCHASE CONTRACT is made on <b>{contract.contDate}</b> between{" "}
        ...
        <table className="w-full text-[11px] mb-4">
          <tbody>
            ...
            <tr>
              ...
              <td className="border border-black p-2 font-bold">Brokerage / Bag</td>
              <td className="border border-black p-2 mono">{fmt(contract.brokagePerBag)}</td>
            </tr>
            <tr>
        ...
        <table className="w-full text-[11px] mb-4">
          <thead>
            <tr>
              ...
              <th className="border border-black p-2 text-left" style={{ background: "#eee" }}>DO No</th>
          ...
          <tbody>
            {deliveries.length === 0 ? (
              <tr>
                <td colSpan={5} className="border border-black p-2 text-center italic">
              ...
              deliveries.map((d, i) => (
                <tr key={d.id}>
                  ...
                  <td className="border border-black p-2 mono">{d.doNo ?? "-"}</td>
