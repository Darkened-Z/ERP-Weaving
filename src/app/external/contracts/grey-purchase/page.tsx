...
export default async function GreyPurchaseContractPage({
  ...
  const contracts = findFilter
    ? await db
        ...
        .where(
          or(
            sql`${schema.extGreyPurContract.contractNo} LIKE ${pat} ESCAPE '\\'`,
            sql`${schema.extGreyPurContract.party} LIKE ${pat} ESCAPE '\\'`,
  ...
  const deliveries = formItem
    ? await db
        ...
        .where(eq(schema.extGreyPurContractDelivery.contractId, formItem.id))
  ...
  async function saveContract(formData: FormData) {
    ...
    const brokagPercentage = num(formData.get("brokag_percentage"));
    ...
    if (quantityMtr == null || quantityMtr <= 0 || ratePerMtr == null || ratePerMtr <= 0) {
      redirect("/external/contracts/grey-purchase" + errQ("qty_rate_required"));
    }
    ...
    if (deliveryTotal > 0 && Math.abs(deliveryTotal - totalQty) > totalQty * 0.05) {
      redirect("/external/contracts/grey-purchase" + errQ("qty_tolerance"));
    }
      ...
      contractId = await db.transaction(async (tx) => {
        ...
        if (isNew) {
          ...
          const [inserted] = await tx
            .insert(schema.extGreyPurContract)
            .values({
              ...data,
              lContNo: (maxL ?? 0) + 1,
              postedDate: nowIso,
            })
            .returning({ id: schema.extGreyPurContract.id });
          cid = inserted.id;
        } else {
          await tx
            .update(schema.extGreyPurContract)
            .set(data)
        ...
        if (cid) {
          await tx
            .delete(schema.extGreyPurContractDelivery)
            .where(eq(schema.extGreyPurContractDelivery.contractId, cid));

          if (deliveryRows.length > 0) {
            await tx.insert(schema.extGreyPurContractDelivery).values(
    ...
    if (uniqueError) {
      ...
      redirect("/external/contracts/grey-purchase" + q);
    }

    revalidatePath("/external/contracts/grey-purchase");
    redirect(`/external/contracts/grey-purchase?id=${contractId}`);
  }
  ...
  async function deleteContract(formData: FormData) {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.delete(schema.extGreyPurContract).where(eq(schema.extGreyPurContract.id, id));
    });

    revalidatePath("/external/contracts/grey-purchase");
    redirect("/external/contracts/grey-purchase");
  }
  ...
  async function deleteDeliveryRow(formData: FormData) {
    ...
    await db
      .delete(schema.extGreyPurContractDelivery)
      .where(eq(schema.extGreyPurContractDelivery.id, deleteId));

    revalidatePath("/external/contracts/grey-purchase");
    redirect(`/external/contracts/grey-purchase?id=${contractId}`);
  }

  const emptyRows = Math.max(DELIVERY_EMPTY_MIN, DELIVERY_EMPTY_MIN);
  ...
  return (
    <Shell active="ext-gpc">
      <div className="animate-in">
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between mb-6 gap-4">
          <div>
            <h1 className="page-title">GRAY PURCHASE CONTRACT</h1>
          ...
          <ExcelExportButton
            ...
            filename="grey-purchase-contracts"
            sheetName="GreyPurContracts"
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 mb-8">
          <div className="border border-black p-4">
            ...
            <form action={saveContract}>
              ...
              <div className="grid grid-cols-4 gap-3 mb-3">
                <div>
                  <label className="label block mb-1">Brokag Percentage</label>
                  <input
                    name="brokag_percentage"
                    type="number"
                    step="any"
                    className="input-box mono"
                    defaultValue={formItem?.brokagPercentage ?? ""}
                  />
                </div>
                <div>
            ...
            <div className="overflow-y-auto scrollbar-thin" style={{ maxHeight: "60vh" }}>
              ...
              {contracts.slice(0, 100).map((c) => {
                ...
                return (
                  <a
                    key={c.id}
                    href={`/external/contracts/grey-purchase?id=${c.id}`}
