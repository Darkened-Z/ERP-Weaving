...
export default async function YarnPurchaseVoucherPrint({
  params,
}: {
  ...
  const [voucher] = await db
    ...
    .where(eq(schema.extYarnPurVoucher.id, vid))
    .limit(1);
  if (!voucher) notFound();
  ...
  const lines = await db
    ...
    .where(eq(schema.extYarnPurVoucherLine.voucherId, vid))
    ...
    <>
      ...
      <div className="max-w-[210mm] mx-auto p-6">
        <div className="no-print flex justify-between items-center mb-4">
          <Link href={`/external/yarn/purchase?id=${vid}`} className="btn btn-outline btn-sm">
            Back
          </Link>
        ...
        <div className="grid grid-cols-2 gap-4 text-[11px] mb-3">
          <div className="border border-black p-2">
            <div className="uppercase text-[9px] tracking-wide font-bold">Supplier</div>
          ...
          <div className="border border-black p-2">
            <div className="grid grid-cols-2 gap-y-1">
              ...
              {voucher.bmParty ? (<><div className="font-bold uppercase text-[9px]">BM Party</div><div>{voucher.bmParty}</div></>) : null}
              ...
              {voucher.lotNo ? (<><div className="font-bold uppercase text-[9px]">Lot No</div><div>{voucher.lotNo}</div></>) : null}
            </div>
          </div>
        </div>
