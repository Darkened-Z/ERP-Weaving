...
export default async function SalContHistoryPage({
  ...
  const contract = contNo
    ? (await db
        ...
        .where(eq(schema.extYarnSalContract.contNo, contNo))
        .limit(1))[0]
    : null;
  ...
  const deliveries = contract
    ? await db
        ...
        .where(eq(schema.extYarnSalContractDelivery.contractId, contract.id))
    : [];
    ...
    <>
      ...
      <div className="rp-wrap">
        ...
        <div className="rp-page">
          ...
          <div className="rp-title">SALES CONTRACT HISTORY</div>
          ...
          {!contract ? (
            ...
            <>
              ...
              <div className="rp-grid">
                ...
                <div className="rp-row"><span className="rp-key">Brokage %</span><span className="rp-val">{contract.brokagePercentage != null ? `${fmt(contract.brokagePercentage, 2)}%` : "—"}</span></div>
              ...
              <table className="rp-table">
                ...
                <tbody>
                  {deliveries.length === 0 ? (
                    <tr><td colSpan={4} className="rp-empty">No deliveries scheduled.</td></tr>
                  ) : (
                    deliveries.map((d, i) => (
                      <tr key={d.id}>
                        <td>{i + 1}</td>
                ...
                {deliveries.length > 0 && (
                  <tfoot>
                    <tr>
                      <td colSpan={2} style={{ textAlign: "right" }}>Total Bags</td>
          ...
          <div className="rp-footer">Computer generated sales contract history.</div>
        </div>
      </div>
    </>
  );
