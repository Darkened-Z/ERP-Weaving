...
export default async function YarnLocationsPage({
  searchParams,
}: {
  searchParams: Promise<{ id?: string; adding?: string; error?: string }>;
}) {
  const params = await searchParams;
  const allLocations = await db.select().from(schema.locations).where(eq(schema.locations.type, "YARN")).orderBy(schema.locations.code);
  ...
  async function saveLocation(formData: FormData) {
    ...
    try {
      if (id) {
        ...
        await db.update(schema.locations).set({ code, description, type: "YARN" }).where(eq(schema.locations.id, parsedId));
        revalidatePath("/define/yarn-locations");
        redirect(`/define/yarn-locations?id=${parsedId}`);
      } else {
        const [row] = await db
          .insert(schema.locations)
          .values({ code, description, type: "YARN" })
          .returning({ id: schema.locations.id });
        revalidatePath("/define/yarn-locations");
        redirect(`/define/yarn-locations?id=${row.id}`);
      }
    } catch (e) {
      if (isUniqueViolation(e)) redirect(`/define/yarn-locations?${id ? `id=${id}&` : ""}error=exists`);
      throw e;
    }
  }
  ...
  async function deleteLocation(formData: FormData) {
    ...
    revalidatePath("/define/yarn-locations");
    redirect("/define/yarn-locations");
  }

  return (
    <Shell active="locations-yarn">
      <div className="animate-in">
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between mb-8 gap-4">
          <h1 className="page-title">
            Yarn Parties Location{" "}
            ...
            <div className="border border-black p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <a href="/define/yarn-locations?adding=1" className="btn btn-outline btn-sm">New</a>
                  {formItem && (
              ...
              <form action={saveLocation}>
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <a href="/define/yarn-locations" className="btn btn-outline btn-sm">Exit</a>
                </div>
              </form>
            </div>
          </div>
              ...
              <table>
                ...
                <tbody>
                  {allLocations.map((l) => {
                    ...
                    const rowHref = `/define/yarn-locations?id=${l.id}`;
