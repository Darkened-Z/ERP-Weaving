...
export default async function CostCenterConsumptionPage({
  searchParams,
}: {
  ...
  const rows = rowsRaw
    .map((r) => ({
      ccCode: r.ccCode ?? "-",
      ccDesc: ccMap.get(r.ccCode ?? "") ?? r.ccCode ?? "Unassigned",
      count: r.count ?? 0,
      qty: r.qty ?? 0,
      amount: r.amount ?? 0,
    }))
    ...
    <Shell active="s-cc">
      <div className="animate-in">
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between mb-6 gap-4 no-print">
          <div>
            <h1 className="page-title">Cost Center Consumption</h1>
            <p className="text-[13px] text-[var(--muted)] mt-2">
              {rows.length} cost centers &middot; {from} to {to}
            </p>
          </div>
          <div className="flex gap-2 items-center">
            ...
            <ExcelExportButton
              ...
              filename="cost-center-consumption"
            />
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-end gap-2">
            ...
            <a href="/reports/store/cost-center-consumption" className="btn btn-outline btn-sm">
              Clear
            </a>
          </div>
        </form>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-black border-2 border-black mb-8 no-print">
          <div className="bg-white p-4">
            ...
            <div className="stat-label">Cost Centers</div>
          </div>
          <div className="bg-white p-4">
                ...
                rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="mono text-right">
                      {totAmt > 0 ? ((r.amount / totAmt) * 100).toFixed(1) : "-"}
                    </td>
                  </tr>
                ))
              )}
